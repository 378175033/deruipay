...
class AlipayTradeService {
	...
	function writeLog($text) {
		...
		//$text = characet ( $text );
		file_put_contents ( dirname(dirname(__FILE__))."/log/log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
...
//		$image = '<img src="http://chart.apis.google.com?chs='.$size.'x'.$size.'&amp;cht=qr&chld='.$lev.'|'.$margin.'&amp;chl='.$content.'"  widht="'.$size.'" height="'.$size.'" />';
		$image = '<img src="http://qr.liantu.com/api.php?bg=f3f3f3&fg=ff0000&gc=222222&el=l&w=200&m=10&text='.$content.'"/>';
