<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:80:"D:\phpStudy\PHPTutorial\WWW\F4\public/../application/manage\view\user\index.html";i:1563431342;s:71:"D:\phpStudy\PHPTutorial\WWW\F4\application\manage\view\common\head.html";i:1568092800;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script type="text/html" id="barDemo">
    ...
    <?php if($user_id == 1): ?>
        ...
        {{# } }}
    <?php else: ?>
        <button class="layui-btn layui-btn-xs layui-btn-normal btn-update layui-btn-disabled"  data-id="{{ d.id }}" title="修改数据" disabled>
            <i class="layui-icon layui-icon-edit"></i>
        </button>
        {{# if( d.id !== 1 ){ }}
        <button class="layui-btn layui-btn-xs layui-btn-danger btn-remove layui-btn-disabled" data-id="{{ d.id }}" title="移除数据" disabled>
            <i class="layui-icon layui-icon-delete"></i>
        </button>
        <button href="javascript:openIframe({content:'auth?id={{d.id}}',area:['30%','80%'],title:'配置权限'});" class="layui-btn layui-btn-xs layui-btn-disabled" title="配置权限" disabled>
            <i class="layui-icon layui-icon-set-fill"></i>
        </button>
        {{# } }}
    <?php endif; ?>
